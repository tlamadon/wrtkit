...
def create_base_config():
    ...
    wan = NetworkInterface("wan").with_device("eth0").with_dhcp()
    ...
    dhcp_lan = DHCPSection("lan").with_interface("lan").with_range(100, 200, "12h")
...
def extend_with_guest_network():
    ...
    guest_vlan = NetworkDevice("guest_vlan").with_type("8021q").with_ifname("lan1").with_vid(100)
    ...
    # Guest DHCP
    guest_dhcp = DHCPSection("guest").with_interface("guest").with_range(50, 100, "2h")
    config.dhcp.add_dhcp(guest_dhcp)

    # Guest WiFi on 2.4GHz
    guest_wifi = (
        WirelessInterface("guest_radio1")
        .with_device("radio1")
        .with_mode("ap")
        .with_network("guest")
    ...
    # Allow guest to wan
    guest_forwarding = FirewallForwarding(1).with_src("guest").with_dest("wan")
...
def add_multiple_aps():
    ...
    for ap_config in aps:
        ap = (
            WirelessInterface(ap_config["name"])
        ...
        for interface in config.network.interfaces:
            if interface._section == "lan":
                ...
                config.network.interfaces = [
                    updated if i._section == "lan" else i for i in config.network.interfaces
                ]
                break
